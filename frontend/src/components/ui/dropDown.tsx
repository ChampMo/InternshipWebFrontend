...
export default function Dropdown({
    ...
    return (  
        <div className='relative' ref={dropdownRef}>
            ...
            <div className={`absolute bg-white border-t-0 pt-2 border-primary1 rounded-b-lg mt-2 w-full right-0 duration-200 ${state ? `${items.length == 1 || items.length == 0 ? 'h-12 overflow-hidden':items.length == 2? 'h-22 overflow-hidden':items.length == 3? 'h-33 overflow-hidden':items.length == 4? 'h-43 overflow-hidden': 'h-53'} overflow-auto shadow-lg border top-7` : 'h-0 overflow-hidden top-0'}`}>
